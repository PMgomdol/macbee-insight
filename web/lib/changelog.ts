...
export const CHANGELOG: Release[] = [
  {
    date: '2026-08-30',
    title: '드라이브 저장 연결 준비 · 한글 파일명 업로드 수정 · 차트 수치',
    changes: [
      { type: 'feature', text: '업로드 파일을 맥비님 구글 드라이브에 저장하는 연결을 준비했어요. 승인이 끝난 자료의 파일만 드라이브로 옮겨지고 자료실 링크가 드라이브 주소로 바뀌어요. 옮기지 못하면 운영진에게 메일이 가고, [자료 관리](/admin-mb26/panel/archive)의 "드라이브로" 버튼으로 다시 시도할 수 있어요. (맥비님 쪽 설정이 끝나면 켜져요 — 그 전엔 지금처럼 동작)' },
