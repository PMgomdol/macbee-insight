...
// size: 'sm' = 카드 한 장 같은 부분 캡처(최대 440px), 기본 = 화면 전체 캡처(본문 폭)
function Shot({ src, alt, caption, size }: { src: string; alt: string; caption?: string; size?: 'sm' }) {
  const ok = shotExists(src);
  return (
    <figure className={`flex flex-col gap-1.5 my-1 ${size === 'sm' ? 'max-w-[440px]' : ''}`}>
      {ok ? (
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      ...
      <Section n="1" title="맥비 자료실이란">
        ...
        <p>
          운영진 업무는 관리 화면 <K>macbe-archive.com/admin-mb26</K> 에서 하고, 권한은 <b>운영진</b>과 <b>관리자</b> 두 단계임(<S n="4" />).
        </p>
      ...
      <Section n="2" title="데이터 구조 — 자료의 구성과 저장">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,260px)_1fr] gap-4 items-start">
          ...
          <Ul>
            <li><b>형식</b> — 카드 왼쪽 위 배지. 아티클 · 가이드 · 기획서 · 템플릿 · 영상 · 세미나</li>
        ...
        <p>링크가 아니라 파일(PDF·PPT·이미지 등, 30MB까지)을 올린 자료는 파일 자체가 따로 보관됨. 흐름은 아래와 같음.</p>
        <Step>
      ...
      <Section n="3" title="등록 원리 — 자료가 게시되기까지">
        <p>
          자료 한 건이 사이트에 오르기까지의 흐름임. 핵심은 <b>제안 → 검수 → 승인 → 게시</b>이고, 모든 과정이 사이트 안에서 끝남.
        </p>
...
대기 목록에 쌓임  ──▶ 운영진에게 "새 제안" 알림 메일
   ...
   ▼
승인  (방문자 제안 = 운영진 2명 / 운영진 등록 건 = 다른 운영진 1명 확인 후 단독 승인)
   │  파일이 붙은 자료는 이때 드라이브로 이동
   ▼
사이트에 게시  ──▶ 제안한 사람에게 "승인" 메일`}</pre>
        <SubHeading>승인 규칙 — 누가 올렸느냐에 따라 다름 (2026-08-31 회의 결정)</SubHeading>
        <Ul>
          <li><b>방문자 제안</b> — 서로 다른 <b>운영진 2명</b>이 승인해야 게시됨. 한 사람이 놓치기 쉬운 중복·품질·적합성을 두 번 거르기 위함.</li>
          <li><b>운영진이 직접 등록한 자료</b> — 다른 운영진 <b>1명</b>이 내용을 확인하고 <K>단독 승인</K>으로 게시함. 운영진 등록임을 알아볼 수 있게 <b>제안자란에 실명을 기입</b>할 것.</li>
        </Ul>
        <p>
          <K>단독 승인</K>은 사유를 적게 되어 있음 — 운영진 등록 건은 &ldquo;운영진 등록 건 확인&rdquo;처럼 남기면 되고, 사유는 자료 기록에 남음.
        </p>
        <Note>제안은 <b>방문자</b>가 할 수도, <b>운영진이 직접</b> 할 수도 있음. 둘 다 같은 대기 목록으로 들어옴(<S n="6" />).</Note>
      </Section>
      ...
      <Section n="4" title="로그인과 권한">
        ...
        <Step>
          ...
          <li>아직 권한이 없으면 <b>운영진 신청 화면</b>이 보임. 신청하면 &ldquo;승인을 기다리고 있어요&rdquo;로 바뀌고, 관리자가 확인해 권한을 주면 다음 로그인부터 관리 화면이 열림(<S n="11" />). 오래 안 열리면 관리자에게 말할 것.</li>
        </Step>
        ...
        <Note>로그인만 하고 권한을 안 받은 사람은 자료를 <b>제안</b>만 할 수 있고, 관리 화면은 못 봄. 2026-08-31부터는 운영진 전원이 관리자 권한을 가짐 (단독 승인 규칙 실행을 위해).</Note>
      </Section>
      ...
      <Section n="5" title="화면 둘러보기">
        ...
        <SubHeading>평소 루틴 — 이것만 하면 됨</SubHeading>
        <Ul>
          <li><b>메일이 오면</b> (새 제안·새 의견) — 관리 화면 홈에 들어가 대기 건 처리 (<S n="6" /> · <S n="8" />). 당일이 좋고 늦어도 2~3일 안에.</li>
          <li><b>주 1회</b> — 대시보드에서 &lsquo;결과가 없던 검색어&rsquo;와 인기 자료 확인 (<S n="12" />), 자료 관리의 &lsquo;중복&rsquo; 탭과 죽은 링크 배지 정리 (<S n="7" />).</li>
          <li><b>생각날 때</b> — 좋은 자료를 보면 <K>자료 등록</K> 폼으로 제안 (<S n="6" />), 할 일은 백로그에 (<S n="9" />).</li>
        </Ul>
      </Section>
      ...
      <Section n="6" title="자료 등록요청 처리">
        ...
        <Step>
          ...
          <li><b>메뉴 확인.</b> 승인하면 <b>받아서 쓰는 자료는 양식·템플릿, 읽거나 보는 자료는 콘텐츠</b>로 자동 배치됨 — 카드의 &lsquo;메뉴&rsquo; 배지가 예상 위치임. <b>메뉴 ?</b>로 표시된 건은 내용을 못 읽어 파일 여부로만 배치된 것이니 한 번 봐줄 것. 파일인데 내용이 읽을거리(칼럼·의견 모음)면 승인 뒤 자료 관리에서 콘텐츠로 바꾸면 됨.</li>
          <li><b>승인.</b> 방문자 제안은 서로 다른 <b>운영진 2명</b>이 승인하면 게시됨. <b>운영진이 등록한 건</b>(제안자란의 실명으로 확인)은 다른 운영진 1명이 내용을 보고 <K>단독 승인</K>으로 게시함.</li>
          <li><b>반려.</b> 자료실에 맞지 않으면 사유를 적고 반려함. 사유는 제안한 사람에게 안내 메일로 전달되니 한 줄이라도 이유를 적을 것 (예: &ldquo;링크가 열리지 않음&rdquo;, &ldquo;이미 등록된 자료&rdquo;, &ldquo;기획 실무와 거리가 있음&rdquo;). 반려한 건은 <S n="7" />의 &lsquo;거절됨&rsquo; 탭에 남고, 마음이 바뀌면 <b>다시 검토</b>로 대기 목록에 되돌릴 수 있음.</li>
        </Step>
        ...
        <p>
          ...
          이때 <b>제안자란에 실명을 적을 것</b> — 운영진 등록임을 알아보고 1명 확인으로 처리하기 위함.
          그러면 위 대기 목록으로 들어오고, 다른 운영진 1명이 확인해 <K>단독 승인</K>으로 게시함.
        </p>
      ...
      <Section n="7" title="자료 관리">
        ...
        <SubHeading>탭별로 할 일</SubHeading>
        <Ul>
          <li><b>중복</b> — 같은 링크를 가진 자료를 묶어 보여줌. 제목·설명이 더 잘 된 쪽 하나만 남기고 나머지는 삭제.</li>
          <li><b>죽은 링크 배지</b> — 매주 자동 점검해 열리지 않는 링크에 배지가 붙음. 링크를 직접 열어 확인한 뒤, 새 주소를 찾으면 <b>수정</b>, 못 찾으면 <b>숨김</b> 또는 삭제. (봇을 막는 사이트는 실제론 멀쩡할 수 있으니 눈으로 확인)</li>
          <li><b>거절됨</b> — 반려한 제안과 사유. 같은 자료가 또 올라오면 여기서 이유 확인. <b>다시 검토</b>로 되돌리기 가능.</li>
        </Ul>
      ...
      <Section n="8" title="의견(VOC) 답변">
        <p>
          ...
          카드를 눌러 상태(신규·처리중·보류·답변완료·종료)를 바꾸고 담당자를 지정할 수 있음. 답이 필요한 의견은 남겨준 이메일로 <b>본인 메일에서 직접</b> 회신하고, 보낸 뒤 상태를 <b>답변완료</b>로 바꿔둘 것 — 그래야 다른 운영진이 중복 회신하지 않음. 이메일 없이 온 의견(수정 요청 등)은 처리 후 종료.
        </p>
      ...
      <Section n="12" title="대시보드 보는 법">
        ...
        <Shot src="/guide-assets/16-dash-popular.png" size="sm" alt="대시보드 인기 자료 카드" caption="콘텐츠 성과 탭 → 인기 자료 (30일 조회)" />
        ...
        <Shot src="/guide-assets/17-dash-zero-search.png" size="sm" alt="대시보드 결과가 없던 검색어 카드" caption="행동·전환 탭 → 결과가 없던 검색어" />
        ...
        <Shot src="/guide-assets/18-dash-traffic.png" size="sm" alt="대시보드 유입 경로 카드" caption="유입 탭 → 유입 경로" />
      ...
      <Section n="14" title="콘텐츠 작성 규칙">
        ...
        <Ul>
          ...
          <li><b>형식</b> — 아티클 · 가이드 · 기획서 · 템플릿 · 영상 · 세미나 중에서 고름. 템플릿 = 복사해 바로 쓰는 양식·샘플, 가이드 = 방법·절차 설명, 기획서 = 실제 PRD·화면설계 문서, 아티클 = 그 외 읽는 글. <b>템플릿만</b> <K>양식·템플릿</K> 메뉴로 가고, 나머지는 <K>콘텐츠</K>로 감.</li>
      ...
      <Section n="19" title="FAQ">
        <div className="flex flex-col gap-2.5">
          ...
          <div><b>Q. 혼자 승인해도 되는 경우</b><br/>운영진이 실명을 적고 등록한 자료는 다른 운영진 1명이 확인 후 단독 승인으로 게시함. 방문자 제안은 원칙대로 서로 다른 2명이 승인(<S n="3" />).</div>
          <div><b>Q. 올린 파일은 어디에 저장되나</b><br/>승인되면 맥비님 구글 드라이브 폴더로 감. 자세한 흐름은 <S n="2" />의 &lsquo;업로드 파일은 이렇게 관리됨&rsquo;.</div>
          <div><b>Q. 로그인했는데 관리 화면이 안 보일 때</b><br/>아직 운영진 권한이 없는 것. 운영진 신청 화면에서 신청하고 관리자에게 알릴 것(<S n="4" />).</div>
          <div><b>Q. AI가 뽑은 제목·설명이 이상할 때</b><br/>승인 전에 등록요청 카드의 <b>수정</b>으로 고치면 됨. 제목은 곧 드라이브 파일명이 되니 이때 정리(<S n="6" /> · <S n="14" />).</div>
          <div><b>Q. 내가 올린 자료를 내가 승인해도 되나</b><br/>안 됨 — 확인은 올린 사람이 아닌 <b>다른 운영진 1명</b>이 함. 그 1명이 단독 승인으로 게시함(<S n="3" />).</div>
